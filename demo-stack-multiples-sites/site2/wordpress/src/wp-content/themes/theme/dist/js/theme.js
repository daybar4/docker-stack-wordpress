...
/*! cam-frontend 2022-09-08 */
// Source: src/theme.js
/*-- CUSTOM JS --*/
var dropdown1 = $('.main_menu_dropdown_trigger');
var dropdownOptions = {
    'coverTrigger': false,
    'hover': true
};
...
if (dropdown1.length !== 0) {
    var instances = M.Dropdown.init(dropdown1, dropdownOptions);
...
$(document).ready(function () {
    ...
    /*SELECT*/
    /*$('select').formSelect();
    $('#lang_select').parents('.select-wrapper').find('input.select-dropdown').attr('id', 'hidden_input').attr('aria-label','Language selector collapsed');*/
    /*DROPDOWN*/
    //TOP BAR
    if (window.innerWidth <= 1019) {
        $('.dropdown-trigger.top_bar_links_trigger').dropdown({
            coverTrigger: false
        });
    }
    if (window.innerWidth > 1019) {
        $('.dropdown-trigger.other_websites_trigger').dropdown({
            coverTrigger: false,
            alignment: 'right'
        });
    }
    ...
    $('.main_menu_trigger').click(function () {
        ...
        $(this).find('.btn_ico').toggleClass('ico_burger').toggleClass('ico_close');
        $('.main_menu').toggleClass('active');
    });
    if (window.innerWidth <= 1019) {
        dropDestroy();
        $('.main_menu_dropdown_trigger').click(function () {
            $(this).toggleClass('active');
            $(this).parents('.mtz:eq(0)').find('.main_menu_dropdown').slideToggle();
        });
    }
    if (window.innerWidth > 1019) {
        $('.sticky_header').on('sticky-start', function () {
            dropDestroy();
            $('.main_menu_dropdown_trigger').click(function () {
                $(this).toggleClass('active');
                $(this).parents('.mtz:eq(0)').find('.main_menu_dropdown').slideToggle();
        ...
        $('.sticky_header').on('sticky-end', function () {
            $('.main_menu_trigger').removeClass('active');
            $('.main_menu_dropdown_trigger').removeClass('active').unbind();
            $('.main_menu_trigger').find('.btn_ico').addClass('ico_burger').removeClass('ico_close');
            $('.main_menu').removeClass('active');
            $('.main_menu_dropdown').removeAttr("style");
            dropInit();
    ...
    //GALLERY_SLIDER
    /*var numSlick = 0;
    $('.gallery_slider_for').each( function() {
        numSlick++;
        $(this).addClass( 'slider-' + numSlick ).slick({
            ...
            pauseOnHover: false,
            infinite: false,
            asNavFor: '.gallery_slider_nav.slider-' + numSlick
        });
    });
    numSlick = 0;
    $('.gallery_slider_nav').each( function() {
        numSlick++;
        $(this).addClass( 'slider-' + numSlick ).slick({
            slidesToShow: 5,
            slidesToScroll: 5,
            asNavFor: '.gallery_slider_for.slider-' + numSlick,
            dots: false,
            arrows: false,
            centerMode: false,
            focusOnSelect: true,
    ...
    var paused = 0;
    $('.gallery_slider_for img').click(function () {
        if (paused === 1) {
            $(this).parents('.gallery_slider_for:eq(0)').slick('slickPlay');
            $(this).parents('.gallery_slider_for:eq(0)').removeClass('paused');
            paused = 0;
        } else {
            $(this).parents('.gallery_slider_for:eq(0)').slick('slickPause');
            $(this).parents('.gallery_slider_for:eq(0)').addClass('paused');
            paused = 1;
        }
    });
    $('.gallery_slider_for').on('afterChange', function (event, slick, currentSlide) {
        $(this).parents('.gallery:eq(0)').find('.gallery_slider_nav').slick('slickGoTo', currentSlide);
        var currrentNavSlideElem = $(this).parents('.gallery:eq(0)').find('.gallery_slider_nav .slick-slide[data-slick-index="' + currentSlide + '"]');
        $(this).parents('.gallery:eq(0)').find('.gallery_slider_nav .slick-slide.is-active').removeClass('is-active');
        $(currrentNavSlideElem).addClass('is-active');
        if (currentSlide === slick.$slides.length - slick.slickGetOption('slidesToShow')) {
    ...
    $('.gallery_slider_nav').on('init', function (event, slick) {
        $(this).find('.slick-slide.slick-current').addClass('is-active');
    }).on('click', '.slick-slide', function (event) {
        event.preventDefault();
        var goToSingleSlide = $(this).data('slick-index');
        $(this).parents('.gallery:eq(0)').find('.gallery_slider_for').slick('slickGoTo', goToSingleSlide);
    }).slick({
        slidesToShow: 5,
        slidesToScroll: 5,
        dots: false,
    ...
    //FOOTER VLINKS
    $('.trigger_vlinks').click(function () {
        $(this).find('span:eq(0)').toggleClass('ico_plus').toggleClass('ico_minus');
        $(this).parents('.vlinks_container:eq(0)').find('.footer_vlinks').slideToggle();
    });
    ...
    $('.faq_question').click(function () {
        ...
        $(this).parents('li:eq(0)').find('.faq_answer').slideToggle();
    });

    /*OBJECT FIT POLLYFILL*/
    objectFitImages('.ob_fit_cover');
});
